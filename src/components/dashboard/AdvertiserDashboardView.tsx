...
import { ArrowRight, CircleDollarSign, Compass, Megaphone, Users, ClipboardList, Clock, Bell, MessageSquare, Pin } from "lucide-react";
  ...
  const creatorsQ = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("user_id, headline, categories, profiles!creator_profiles_user_id_fkey!inner(display_name, avatar_url, location)")
        .order("created_at", { ascending: false })
        .limit(6);
      return data ?? [];
    },
